...
public class AuthorizeEndPoint extends HttpServlet {
    ...
    Logger logger = (Logger) LoggerFactory.getLogger(AuthorizeEndPoint.class);  //LoggerFactory determines the calling Class Name through reflection.
    ...
    @RequestMapping(value = "/launchpatient", method = RequestMethod.GET)     // This API Work
    @ResponseBody
    ...
    public void authorizeAfterLaunchPatient(HttpServletRequest request, HttpServletResponse response, @RequestParam("transaction_id") String transactionId, @RequestParam("id") Integer launchPatientId) throws IOException, ServletException {
        ...
        String url = tempAuth.getRedirect_uri().trim() + "?code=" + tempAuth.getAuthCode().trim() + "&state=" + tempAuth.getState().trim();    //How to form this
