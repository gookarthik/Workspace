...
public class MedicationStatementResourceProvider implements IResourceProvider {
    ...
    private MedicationStatement createMedicationStatementObject(DafMedicationStatement dafMedStatement) {
        ...
        //set Status
        medStatement.setStatus(MedicationStatementStatusEnum.valueOf(dafMedStatement.getStatus().trim().toUpperCase()));
...
//        medStatement.setMedication(referenceDt);
        ...
        timeRepeat.setPeriodUnits(UnitsOfTimeEnum.valueOf(dosageStatement.get("timingperiodunits").trim().toUpperCase()));
        ...
        if(dosageStatement.get("dosagetext")!=null) {
    ...
    @Create
    public MethodOutcome medStToDaf(@ResourceParam MedicationStatement theMedicationStatement ) {
    	
    	MethodOutcome retVal = new MethodOutcome();
    	try {
    		
    		DafMedicationStatement dafMed = MedStToDaf(theMedicationStatement);
    		retVal.setId(new IdDt("1"));
    		
    		DafMedicationStatement daf = service.saveMedSt(dafMed);
    	}catch(Exception e) {
    		System.out.println(e.getMessage());
    		e.printStackTrace();
    	}
    	System.err.println("save MedicationStatement done!!!");
		return retVal;
    }

	private DafMedicationStatement MedStToDaf(MedicationStatement theMedicationStatement) {
		
		System.err.println("in MedStToDaf");
		DafMedicationStatement daf = new DafMedicationStatement();
		
		LinkedHashMap<String, String>h1 = new LinkedHashMap<String, String>();
//		h1.put("routedisplay",theMedicationStatement.getDosage().get(0).getRoute().getCoding().get(0).getDisplay().toString());
//		h1.put("timingperiod", theMedicationStatement.getDosage().get(0).getTiming().getRepeat().getPeriod().toString());
//		h1.put("dosagetext", theMedicationStatement.getDosage().get(0).getText().toString());
//		h1.put("routecode", theMedicationStatement.getDosage().get(0).getRoute().getCoding().get(0).getCode());
//		h1.put("timingperiodunits", theMedicationStatement.getDosage().get(0).getTiming().getRepeat().getPeriodUnits().toString());
		
		
//		daf.setPatient(theMedicationStatement.getPatient().getReference().toString());
		daf.setWasnottaken(theMedicationStatement.getWasNotTaken().booleanValue());
//		daf.setReasonnottaken(theMedicationStatement.getReasonNotTaken().get(0).getCoding().get(0).getDisplay().toString());
//		daf.setEffectivePeriod(theMedicationStatement.getEffective().toString());
		daf.setStatus(theMedicationStatement.getStatus().toString());
//		daf.setDosage((JJ.jajn(h1)));
