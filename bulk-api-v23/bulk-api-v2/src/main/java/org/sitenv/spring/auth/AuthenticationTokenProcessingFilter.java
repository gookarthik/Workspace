...
@WebServlet  //It is to declare a servlet
public class AuthenticationTokenProcessingFilter extends GenericFilterBean {  // which treats its config parameters as bean properties

    @Autowired
    private AuthTempDao dao;
	
    @Override
    public void doFilter(ServletRequest request, ServletResponse response,                    //This is invoked when user tries to access a secured REST resource without supplying any credentials
        ...
        if (authToken != null && "bearer".equalsIgnoreCase(authToken.split(" ", 2)[0].toString())) {	// why bearer
            ...
            if (authentication != null && authToken.equalsIgnoreCase(authentication.getAccess_token())) {
                try {
                    ...
                    if (Common.convertTimestampToUnixTime(expiryTime) + 900 > currentTime) {  //Why 900
                        ...
                        httpResponse.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Unauthorized: Authentication token is expired.");//We should just send a 401 Unauthorized response because there is no 'login page' to redirect to
                    }
        ...
        } else if (httpRequest.getRequestURI().contains("/open-fhir/")) {		// Where and How to use all these
            SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken("user", "password", authorities));
            chain.doFilter(request, response);
