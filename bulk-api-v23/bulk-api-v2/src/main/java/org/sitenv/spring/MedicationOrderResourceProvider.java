...
//        medOrder.setMedication(classCodeDt);
    ...
    @Create
    public MethodOutcome SaveToMedOrd(@ResourceParam MedicationOrder theMedicationOrder) {
    	
    	MethodOutcome retVal = new MethodOutcome();
    	try {
    		    	
    		DafMedicationOrder dafMed = medOrdToDaf(theMedicationOrder);
    		retVal.setId(new IdDt("1"));
    	
    		DafMedicationOrder daf = service.saveToMedOrd(dafMed);
	...
	private DafMedicationOrder medOrdToDaf(MedicationOrder theMedicationOrder) {
		
		System.err.println("in medOrdToDaf");
		DafMedicationOrder daf = new DafMedicationOrder();
		
		LinkedHashMap<String, String> h1 = new LinkedHashMap<>();
//		h1.put("doseunit", theMedicationOrder.getDosageInstruction().get(0)
		h1.put("routedisplay",theMedicationOrder.getDosageInstruction().get(0).getRoute().getCoding().get(0).getDisplay().toString());
		h1.put("timingperiod", theMedicationOrder.getDosageInstruction().get(0).getTiming().getRepeat().getPeriod().toString());
		h1.put("text", theMedicationOrder.getDosageInstruction().get(0).getText().toString());
//		h1.put("dosesystem", theMedicationOrder.getDosageInstruction().get(0).getMaxDosePerPeriod(). )
		h1.put("timingfrequency",theMedicationOrder.getDosageInstruction().get(0).getTiming().getRepeat().getFrequency().toString() );
		h1.put("routecode", theMedicationOrder.getDosageInstruction().get(0).getRoute().getCoding().get(0).getCode().toString());
		h1.put("timingperiodunits", theMedicationOrder.getDosageInstruction().get(0).getTiming().getRepeat().getPeriodUnits().toString());
		
		
		LinkedHashMap<String, String> h2 = new LinkedHashMap<>();
		h2.put("quantityunit", theMedicationOrder.getDispenseRequest().getQuantity().getUnit().toString());
		h2.put("quantityvalue", theMedicationOrder.getDispenseRequest().getQuantity().getValue().toString());
//		h2.put("validity", theMedicationOrder.getDispenseRequest().getValidityPeriod().getStart().toString()+ "  "+theMedicationOrder.getDispenseRequest().getValidityPeriod().getEnd().toString());
		h2.put("quantitycode", theMedicationOrder.getDispenseRequest().getQuantity().getCode().toString());
		h2.put("repeatFrequency", theMedicationOrder.getDosageInstruction().get(0).getTiming().getRepeat().getFrequency().toString());
		h2.put("quantitysystem", theMedicationOrder.getDispenseRequest().getQuantity().getSystem().toString());
		
		
		
		
		daf.setDateWritten(theMedicationOrder.getDateWritten());
//		daf.setPatient(theMedicationOrder.getPatient().getReference());;
//		daf.setPrescriber(theMedicationOrder.getPrescriber().getReference());;
//		daf.setMedicationreference(theMedicationOrder.getme);
		daf.setDosageinstruction(JJ.jajn(h1));
		daf.setDispencerequest(JJ.jajn(h2));
		daf.setStatus(theMedicationOrder.getText().getStatus().toString());
		daf.setIdentifierSystem(theMedicationOrder.getIdentifier().get(0).getSystem());
		daf.setIdentifierValue(theMedicationOrder.getIdentifier().get(0).getValue());
		
		System.err.println("Coming out of medOrdToDaf");	
